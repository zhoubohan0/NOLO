...
class LanguageBindVideoProcessor(ProcessorMixin):
    ...
    def __call__(self, images=None, text=None, context_length=77, return_tensors=None, **kwargs):
        ...
        if images is not None:
            ...
            # image_features = [self.image_processor(image, self.transform,video_decode_backend=self.config.vision_config.video_decode_backend,num_frames=self.config.vision_config.num_frames) for image in images]
            # image_features = torch.stack(image_features)

            video_data = torch.ByteTensor(images).permute(3, 0, 1, 2)  # (T, H, W, C) -> (C, T, H, W)
            image_features = self.transform(video_data).unsqueeze(0)   
